...
String aw_char_to_string(char * name){
    char buffer[80];
    sprintf(buffer, "%s", name);
    return String(buffer);
}

String wifi_id_get_value(char * search){
    String ret = "";
   for (int i=0; i<wifi_id_getValue_count; i++) {

        if (search == wifi_id_getValue_list[i].name) {
            ret =  wifi_id_getValue_list[i].func_1();
            break;
        }
   } 
   return ret;
...
boolean wfifi_connect_station(char* ssid, char* password, IPAddress ip, int mod, boolean savId, boolean cfgIp){
    ...
    delay(20);
        ...
        sprintf(buffer, "host:%s\npswd:%s",ssid, password);
            ...
            display.setTextColor(WHITE);   
            display.setCursor(0, 20);
            display.println("Connection en cours");    
        #endif
    ...
    while (WiFi.status() != WL_CONNECTED)  {
        ...
        Serial.printf(".");
                ...
                display.drawString(0, 0, "WIFI");
                display.setFont(Dialog_plain_11); 
                display.drawString(0, 20, "Connection en cours");
